...
<div class="bgColor" :style="'height:'+fullHeight+'px;'">
  ...
  <div id="mainDriver1" :style="{width: '100px', height: '100px'}"></div>
  <div id="mainDriver2" :style="{width: '100px', height: '100px'}"></div>
  <div id="mainDriver3" :style="{width: '100px', height: '100px'}"></div>
  <div id="mainDriver4" :style="{width: '120px', height: '120px'}"></div>
  ...
  <div style="position:absolute; left: 78%;bottom: 21%; color: white;z-index:99999;font-weight: bolder;font-size: 15px">
    <span>
      {{'职工总数: ' + psnAccount}}
    </span>
  </div>
...
<script>
import MoveTable from '../HomeScreen/MoveTable'
export default {
  name: 'Leader',
  filters: {
  ...
  methods: {
    ...
    initWebSocket () {
      ...
      // 此屏相当于显示器 不发送消息 只做接受消息处理
      // 客户端接收服务端数据时触发
      vm.websocket.onmessage = function (evt) {
        const obj = JSON.parse(evt.data)
        if (obj.type === 'OpenMonth') {
          vm.addMonth(obj.num)
        }
        // switch (obj.Command) {
        //   case '8801':
